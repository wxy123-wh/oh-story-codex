...
- [ ] 情绪通过动作落地（不是直接说"他很难过"）
- [ ] 标题行以外没有章节/写作元信息：不得出现 `事件库|全局状态|写作规则|第[一二三四五六七八九十百千万两0-9]+章|上一章|上章|前一章|本章|这一章|前文|后文|伏笔|细纲|读者` 这类词；需要承接前文时改成角色能感知的事件锚点或相对时间；故事内真实阅读/讨论“第X章”或真实读者身份语境除外
...
| 章节字数偏离目标 | **high** | 写作修复时先回到 `事件库.md` 的当前章节切片补足正文级素材，再展开；去AI味已有正文时不得新增剧情 |
